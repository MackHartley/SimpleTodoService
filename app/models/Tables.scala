...
trait Tables {
   ...
   *  @param todoText Database column todo_text SqlType(text) */
  case class ItemsRow(itemId: Int, userId: Option[Int] = None, todoText: String)
  /** GetResult implicit for fetching ItemsRow objects using plain SQL queries */
  implicit def GetResultItemsRow(implicit e0: GR[Int], e1: GR[Option[Int]], e2: GR[String]): GR[ItemsRow] = GR{
    prs => import prs._
  ...
  class Items(_tableTag: Tag) extends profile.api.Table[ItemsRow](_tableTag, "items") {
    ...
    def ? = ((Rep.Some(itemId), userId, Rep.Some(todoText))).shaped.<>({r=>import r._; _1.map(_=> ItemsRow.tupled((_1.get, _2, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column todo_text SqlType(text) */
    val todoText: Rep[String] = column[String]("todo_text")
